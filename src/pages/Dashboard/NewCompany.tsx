...
const NewCompany: Component = () => {
  ...
  const [desingers, setDesingers] = createSignal<userObject[]>([]);
  const [selectedDesingers, setSelectedDesingers] = createSignal<userObject>();
  ...
  const getAllUsers = async (): Promise<void> => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        const updatedData: userObject[] = docSnap
          .data()
  ...
  const save = async (): Promise<void> => {
    const values = {
      ...
      creator: selectedDesingers() as userObject
    };
    console.log(values);
    ...
    try {
      ...
      await updateDoc(doc(fireStore, 'users', values.creator.value), {
  ...
  // for handling select box
  const format = (item: userObject, type: string) => (type === 'option' ? item.label : item.value);
